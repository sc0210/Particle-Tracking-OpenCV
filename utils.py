...
def Track(SrcFolder, DstFolder, GroupIndex, SavePlot=True):
    ...
    template = ReadGrayImg(f"{SrcFolder}/{GroupIndex}/{sorted (os.listdir(tempPath))[ROI_INDEX]}", False)        
    ...
    data.to_excel(f"{DstFolder}/{GroupIndex}/Result_{GroupIndex}.xlsx", sheet_name='XY', index=False)
...
def SeqNCC(TrackX,TrackY,idx,root,DstFolder,GroupIndex,image_name):
    ...
    plt.savefig(f"{DstFolder}/{GroupIndex}/{image_name}.png", bbox_inches='tight')
    plt.cla()
    plt.close(fig)    
    return x,y
...
def Track2(SrcFolder,DstFolder,GroupIndex):
    ...
    for image in sorted(os.listdir(root))[1:]:
        ...
        if prev_idx < len(os.listdir(root)):
            x,y = SeqNCC(TrackX, TrackY, prev_idx,root,DstFolder,GroupIndex, image) # idx stands for the former one (ROI)
            #print(f"x:{x}, y:{y}")
            TrackX.append(x)
            TrackY.append(y)
